...
function normalizeSettings(raw){
 ...
 out.brandGroups=Array.isArray(raw&&raw.brandGroups)?raw.brandGroups.map((g,i)=>({id:String(g.id||`group_${i+1}`),name:String(g.name||`Brand Group ${i+1}`),enabled:g.enabled!==false,brands:Array.isArray(g.brands)?[...g.brands]:[],preferredBrand:String(g.preferredBrand||''),stockCreditPct:Number.isFinite(+g.stockCreditPct)?+g.stockCreditPct:100,onWayCreditPct:Number.isFinite(+g.onWayCreditPct)?+g.onWayCreditPct:100,onWay2CreditPct:Number.isFinite(+g.onWay2CreditPct)?+g.onWay2CreditPct:100})):[];
 return out;
}
...
function renderDashboard(){
 ...
 const statuses=['CRITICAL ORDER','REORDER SOON','WAIT INCOMING','OK','OVERSTOCK','DORMANT / REVIEW','ONE-TIME / REVIEW','DEAD STOCK','NO SALES / REVIEW','NO STOCK / NO PURCHASE — REVIEW'];
...
const plannerColumns=[
 ['internalRef','Internal Ref'],['description','Add Description'],['make','Make'],['oem','OEM'],['brand','Brand'],['brandPartNo','Brand Part No.'],['equivalentNote','Same OEM Other Brands'],['familyPlanNote','Family Purchase Plan'],
 ['suggestedQty','Suggested Qty (Editable)'],['action','Purchase Action'],
 ['allCompany','All Company Qty'],['onWay','On Way'],['onWay2','On Way 2'],['currentStockoutDate','Current Stock-Out'],['pipelineStockoutDate','With Incoming Stock-Out'],['leadTimeDays','Lead Time Days'],['orderCycleDays','Order Cycle Days'],['totalSales','Sales Qty'],['salesCount','Sales Count'],['activeMonths','Sales Months'],
...
function plannerCell(p,k){
 ...
 if(['allCompany','onWay','onWay2','leadTimeDays','orderCycleDays','totalSales','salesCount','totalPurchase','purchaseCount'].includes(k))return`<td class="num">${fmt(v)}</td>`;
 ...
 if(k==='familyPlanNote')return`<td>${p.familyPlanId?`<button class="mini-link open-detail" data-id="${esc(p._id)}">${esc(v)}</button>`:'—'}</td>`;
...
function openDetail(id){
 ...
 $('modalBody').innerHTML=`
  <div class="detail-grid">
   ...
   <div class="metric"><div class="m-label">Brand Order Cycle</div><div class="m-value">${fmt(p.orderCycleDays)} days</div></div>
   <div class="metric"><div class="m-label">Enabled Supplier Horizon</div><div class="m-value">${fmt(p.supplierHorizonDays)} days</div></div>
...
function exportRows(rows,name){
 ...
 const data=rows.map(p=>{
   const out={
    ...
    'Brand Lead Time Days':p.leadTimeDays,'Brand Order Cycle Days':p.orderCycleDays,'Enabled Supplier Horizon Days':p.supplierHorizonDays,'Expected New Order Arrival':dateFmt(p.expectedNewOrderArrivalDate),'MOQ Setting':p.moq,'Order Multiple Setting':p.orderMultiple,
    'Family Purchase Plan':p.familyPlanNote||'','Family Plan Role':p.familyPlanId?(p.familyPlanLead?'PURCHASE LEAD':'FAMILY COVERED'):'','Family Combined Demand':p.familyCombinedDemand||0,'Family Effective Supply':p.familyProjectedSupply||0,'Family Suggested Qty':p.familySuggestedQty||0,'Pre-Family Condition':p.preFamilyCondition||'','Pre-Family Suggested Qty':p.preFamilySuggestedQty||0,
...
function groupDraftCopy(groups){return (groups||[]).map((g,i)=>({id:String(g.id||`group_${Date.now()}_${i}`),name:String(g.name||`Brand Group ${i+1}`),enabled:g.enabled!==false,brands:[...(g.brands||[])],preferredBrand:String(g.preferredBrand||''),stockCreditPct:Number.isFinite(+g.stockCreditPct)?+g.stockCreditPct:100,onWayCreditPct:Number.isFinite(+g.onWayCreditPct)?+g.onWayCreditPct:100,onWay2CreditPct:Number.isFinite(+g.onWay2CreditPct)?+g.onWay2CreditPct:100}));}
...
function renderBrandGroupsUI(){
 ...
 root.querySelectorAll('.brand-group-card').forEach(card=>{
  ...
  const preferred=card.querySelector('.group-preferred');preferred.onchange=e=>g.preferredBrand=groupBrandKey(e.target.value);
  ...
  const refreshPreferred=()=>{const current=groupBrandKey(g.preferredBrand);preferred.innerHTML=`<option value="">Auto — highest-demand available brand</option>${(g.brands||[]).map(k=>`<option value="${esc(groupBrandKey(k))}" ${current===groupBrandKey(k)?'selected':''}>${esc(brandMap.get(groupBrandKey(k))||k)}</option>`).join('')}`;if(current&&!(g.brands||[]).map(groupBrandKey).includes(current)){g.preferredBrand='';preferred.value='';}};
  menu.querySelectorAll('.multi-option input').forEach(cb=>cb.onchange=()=>{const set=new Set((g.brands||[]).map(groupBrandKey));cb.checked?set.add(groupBrandKey(cb.value)):set.delete(groupBrandKey(cb.value));g.brands=[...set];refreshLabel();refreshPreferred();});
...
function addBrandGroup(preset=false){
 ...
 state.brandGroupDrafts.push({id:`group_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,name:preset?'TIBAO FAMILY':`Brand Group ${state.brandGroupDrafts.length+1}`,enabled:true,brands:selected,preferredBrand:preset&&selected.includes('TIBAO')?'TIBAO':'',stockCreditPct:100,onWayCreditPct:100,onWay2CreditPct:100});
 renderBrandGroupsUI();
...
function renderBrandRulesUI(){
 ...
 if(!rows.length){table.innerHTML='<tbody><tr><td colspan="5" class="empty">Upload an Excel file to configure brand-specific purchasing rules.</td></tr></tbody>';return;}
 const s=state.settings;
 table.innerHTML=`<thead><tr><th>Brand</th><th class="num">Lead Time Days</th><th class="num">Order Cycle Days</th><th class="num">MOQ (optional)</th><th class="num">Order Multiple (optional)</th></tr></thead><tbody>${rows.map(([key,name])=>{
  const r=(s.brandRules||{})[key]||{};
  const lead=Number.isFinite(+r.leadTimeDays)?+r.leadTimeDays:s.defaultLeadTimeDays;
  const cycle=Number.isFinite(+r.orderCycleDays)?+r.orderCycleDays:s.defaultOrderCycleDays;
...
function syncSettingsUI(){
 ...
 $('sUseLeadTime').value=s.useLeadTime?'on':'off';$('sDefaultLead').value=s.defaultLeadTimeDays;$('sUseOrderCycle').value=s.useOrderCycle?'on':'off';$('sDefaultOrderCycle').value=s.defaultOrderCycleDays;$('sApplyConstraints').value=s.applyOrderConstraints?'on':'off';$('sDefaultMOQ').value=s.defaultMOQ;$('sDefaultMultiple').value=s.defaultOrderMultiple;
...
function applySettingsFromUI(){
 ...
 state.settings.useLeadTime=$('sUseLeadTime').value==='on';state.settings.defaultLeadTimeDays=Math.max(0,+$('sDefaultLead').value||0);state.settings.useOrderCycle=$('sUseOrderCycle').value==='on';state.settings.defaultOrderCycleDays=Math.max(0,+$('sDefaultOrderCycle').value||0);state.settings.applyOrderConstraints=$('sApplyConstraints').value==='on';state.settings.defaultMOQ=Math.max(0,+$('sDefaultMOQ').value||0);state.settings.defaultOrderMultiple=Math.max(1,+$('sDefaultMultiple').value||1);
 ...
 document.querySelectorAll('#brandRulesTable .rule-input').forEach(inp=>{
  ...
  if(field==='leadTimeDays'||field==='orderCycleDays')rules[key][field]=Math.max(0,+inp.value||0);
 ...
 const groups=groupDraftCopy(state.brandGroupDrafts).map((g,i)=>{const brands=Array.from(new Set((g.brands||[]).map(groupBrandKey).filter(Boolean)));const pref=groupBrandKey(g.preferredBrand);return{id:g.id||`group_${i+1}`,name:(g.name||`Brand Group ${i+1}`).trim(),enabled:g.enabled!==false,brands,preferredBrand:brands.includes(pref)?pref:'',stockCreditPct:Math.max(0,Math.min(100,+g.stockCreditPct||0)),onWayCreditPct:Math.max(0,Math.min(100,+g.onWayCreditPct||0)),onWay2CreditPct:Math.max(0,Math.min(100,+g.onWay2CreditPct||0))};}).filter(g=>g.brands.length>=2);
