...
$('exportTop').onclick=$('exportFiltered').onclick=()=>exportRows(state.filtered,`Purchase_Suggestions_v3_4_${dateFmt(state.reportDate||new Date())}.xlsx`);$('exportSelected').onclick=()=>exportSelectedPurchase(state.filtered.filter(p=>state.selected.has(p._id)),`Selected_Purchase_Order_v3_4_${dateFmt(state.reportDate||new Date())}.xlsx`);
...
$('shjExportFiltered').onclick=()=>exportShjRows(state.shjFiltered,`SHJ_Replenishment_Filtered_v3_4_${dateFmt(state.reportDate||new Date())}.xlsx`);$('shjExportSelected').onclick=()=>exportShjRows(state.shjFiltered.filter(p=>state.shjSelected.has(p._id)),`SHJ_Replenishment_Selected_v3_4_${dateFmt(state.reportDate||new Date())}.xlsx`);$('shjClearSelection').onclick=()=>{const had=state.shjSelected.size;state.shjSelected.clear();renderShj();updateShjSelectionButtons();toast(had?'SHJ selection cleared':'No SHJ items were selected');};
